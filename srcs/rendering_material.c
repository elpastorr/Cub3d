...
/*   Updated: 2023/01/23 17:01:35 by elpastor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	vars->north = create_pixel_tab(vars, &vars->textures[north]);
	vars->south = create_pixel_tab(vars, &vars->textures[south]);
	vars->west = create_pixel_tab(vars, &vars->textures[west]);
	vars->east = create_pixel_tab(vars, &vars->textures[east]);
}
